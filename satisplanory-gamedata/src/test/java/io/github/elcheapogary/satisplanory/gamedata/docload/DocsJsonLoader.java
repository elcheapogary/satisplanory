            ...
            for (JsonObject object : nativeClassesArray.getValuesAs(JsonObject.class)){
                ...
                if (nativeClassName != null && nativeClassName.endsWith("'/Script/FactoryGame.FGRecipe'")){
                    ...
                    for (JsonObject jsonRecipe : classesArray.getValuesAs(JsonObject.class)){
                        ...
                        craftingTimeSeconds = BigDecimalUtils.normalize(craftingTimeSeconds);
                        ...
                        Recipe.Builder recipeBuilder = new Recipe.Builder()
                                .setName(displayName)
                                .setClassName(jsonRecipe.getString("ClassName"))
                                .setCycleTimeSeconds(craftingTimeSeconds)
                                .setManufacturer(producedIn)
                                .setVariablePowerConstant(BigDecimalUtils.normalize(new BigDecimal(jsonRecipe.getString("mVariablePowerConsumptionConstant"))))
                                .setVariablePowerFactor(BigDecimalUtils.normalize(new BigDecimal(jsonRecipe.getString("mVariablePowerConsumptionFactor"))));

                        for (Pair<Item, BigDecimal> p : ingredients){
                            recipeBuilder.addIngredient(p.key(), p.value());
                        }

                        for (Pair<Item, BigDecimal> p : products){
            ...
            throws BracketObjectNotationParseException
    {
        ...
        for (BONObject o : BracketObjectNotation.parseArray(s, BracketObjectNotation::parseObject)){
            ...
            if (item.getMatterState() != MatterState.SOLID){
                decimalAmount = decimalAmount.divide(BigDecimal.valueOf(1000), 3, RoundingMode.UNNECESSARY);
                decimalAmount = BigDecimalUtils.normalize(decimalAmount);
            }

            retv.add(new Pair<>(item, decimalAmount));
        }
