...
public class Straight {
	...
	public Straight(Vector position, Vector direction) {
		if (direction.isNull()) {
			throw new IllegalArgumentException("direction has to be unequal to (0,0)"); //$NON-NLS-1$
		}
	...
	public boolean intersects(Straight other) {
		return direction.getDotProduct(other.direction.getOrthogonalComplement()) != 0;
	}
	 ...
	 * @return true if the true straights intersect and the intersection point is
	 *         contained within the specified segment, false otherwise.
	 * @since 3.2
	 */
	public boolean intersectsWithinSegment(Vector segmentStart, Vector segmentEnd, Straight other) {
		...
		if (!contains(segmentStart) || !contains(segmentEnd)) {
			throw new IllegalArgumentException("segment points have to be contained"); //$NON-NLS-1$
		}
	...
	public Vector getIntersection(Straight other) {
		...
		Vector s1 = direction.getMultiplied(other.position.getDotProduct(other.direction.getOrthogonalComplement()));
		Vector s2 = other.direction.getMultiplied(position.getDotProduct(direction.getOrthogonalComplement()));
		return s1.getSubtracted(s2).getDivided(direction.getDotProduct(other.direction.getOrthogonalComplement()));
	}
	...
	public Vector getProjection(Vector vector) {
		return getIntersection(new Straight(vector, direction.getOrthogonalComplement()));
	}
